...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  /* (STUDENT TODO) Your code goes here... */
    ...
    while (std::getline(file, line)) {
        auto tmp = split(line, ',');
        courses.push_back({tmp[0], tmp[1], tmp[2]});
    }
}
...
// 这里函数参数记得引用传递，否则all_courses中的课程后面没有删除干净，导致下一步函数无法直接输出
void write_courses_offered(std::vector<Course>& all_courses) {
  /* (STUDENT TODO) Your code goes here... */
    std::ofstream out("student_output/courses_offered.csv");
    if (!out) {
        std::cerr << "Error opening file: student_output/courses_offered.csv" << std::endl;
        return ;
    }
    out << "Title,Number of Units,Quarter" << std::endl;
    std::vector<Course> courses_delete;
    for (auto course: all_courses) {
        if (course.quarter != "null") {
            out << course.title << "," << course.number_of_units << "," << course.quarter << std::endl;
            courses_delete.push_back(course);
        }
    }
    for (auto course: courses_delete) {
        //utils中的函数，直接调用即可
        delete_elem_from_vector(all_courses, course);
    }
    out.close();
    // all_courses.erase(std::remove_if(all_courses.begin(), all_courses.end(), 
    //                   [](const Course& x) { return x.quarter == "null";}), all_courses.end());
}
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
    std::ofstream out("student_output/courses_not_offered.csv");
    if (!out) {
        std::cerr << "Error opening file: student_output/courses_not_offered.csv" << std::endl;
        return ;
    }
    out << "Title,Number of Units,Quarter" << std::endl;
